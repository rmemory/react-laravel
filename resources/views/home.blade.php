	...
	<div class="row">
		...
		<div class="col-md-4">
			...
			@foreach($following as $user)
				<span><a href="{{ action('UserController@index', compact('user')) }}" class="btn btn-sm btn-outline-primary">{{ $user->username }}</a></span>
			@endforeach
		</div>
		...
		<div class="col-md-4">
			...
			@foreach($followers as $user)
				<span><a href="{{ action('UserController@index', compact('user')) }}" class="btn btn-sm btn-outline-success">{{ $user->username }}</a></span>
			@endforeach
		</div>
	</div>
</div>
